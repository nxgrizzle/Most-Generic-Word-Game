...
export default function NewGame(props) {
    ...
    const style={
        ...
        content:{display:"grid", gridTemplateColumns:`${props.width < 800 ? "1fr" : "repeat(3, 1fr)"}`, gridGap:"10px", position: "absolute", top: "50%", left: "50%",
        transform: "translate(-50%, -50%)", background:"white", width:"70vw", height:`${props.width < 800 ? "80vh" : "40vh"}`}
    }
    return (
    <>
    <div className="btn" onClick={e=>props.setModal(prev=>({...prev, newGame:true}))}>New Game</div>
        <Modal appElement={document.getElementById("root") || undefined} closeTimeoutMS={400} style={style} isOpen={props.modal.newGame} onRequestClose={()=>props.setModal(prev=>({...prev, newGame:false}))} shouldCloseOnOverlayClick={props.modal.overlayClick} shouldCloseOnEsc={props.modal.escClick}>
